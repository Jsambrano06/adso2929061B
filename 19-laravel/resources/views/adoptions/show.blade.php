...
@section('content')
    ...
    <div class="breadcrumbs text-sm text-white mb-6">
        <ul>
            ...
            <li>
                <span class="inline-flex items-center gap-2">Show Adoption</span>
            </li>
        </ul>
    </div>
    {{-- Card --}}
    <div class="bg-[#0009] p-10 rounded-sm flex flex-col items-center">
        ...
        <div class="mt-4 text-center">
            <div class="badge badge-outline badge-info gap-1">
                ...
                {{ \Carbon\Carbon::parse($adopt->created_at)->format('d/m/Y') }}
            </div>
            <p class="text-white/50 text-xs mt-1">Hace {{ \Carbon\Carbon::parse($adopt->created_at)->diffForHumans() }}</p>
        </div>
        ...
        <div class="flex gap-2 flex-col md:flex-row mt-4">
            <ul class="list bg-[#0006] mt-4 text-white rounded-box shadow-md w-64">
                <li class="list-row">
                    <span class="text-[#fff9] font-semibold">Document:</span> <span>{{ $adopt->user->document ?? 'N/A' }}</span>
                </li>
                <li class="list-row">
                    <span class="text-[#fff9] font-semibold">FullName:</span> <span>{{ $adopt->user->fullname ?? 'N/A' }}</span>
                </li>
                <li class="list-row">
                    <span class="text-[#fff9] font-semibold">Gender:</span> <span>{{ $adopt->user->gender ?? 'N/A' }}</span>
                </li>
                <li class="list-row">
                    <span class="text-[#fff9] font-semibold">Age:</span> <span>{{ \Carbon\Carbon::parse($adopt->user->birthdate)->age }} years old</span>
                </li>
                <li class="list-row">
                    <span class="text-[#fff9] font-semibold">Phone:</span> <span>{{ $adopt->user->phone ?? 'N/A' }}</span>
                </li>
                <li class="list-row">
                    <span class="text-[#fff9] font-semibold">Email:</span> <span>{{ $adopt->user->email ?? 'N/A' }}</span>
                </li>
                <li class="list-row">
                    <span class="text-[#fff9] font-semibold">Active:</span>
                    <span>
                        @if (isset($adopt->user->active) && $adopt->user->active == 1)
                ...
                <li class="list-row">
                    ...
                    <span>
                        @if (isset($adopt->user->role) && $adopt->user->role == 'Admin')
            ...
            <ul class="list bg-[#0006] mt-4 text-white rounded-box shadow-md w-64">
                <li class="list-row">
                    <span class="text-[#fff9] font-semibold">Name:</span> <span>{{ $adopt->pet->name ?? 'N/A' }}</span>
                </li>
                <li class="list-row">
                    <span class="text-[#fff9] font-semibold">Kind:</span> <span>
                        @if(isset($adopt->pet->kind))
                            @if($adopt->pet->kind == 'Perro')
                                <div class="badge badge-outline badge-info">Dog</div>
                            @elseif ($adopt->pet->kind == 'Gato')
                                <div class="badge badge-outline badge-secondary">Cat</div>
                            @else
                                <div class="badge badge-outline">{{ $adopt->pet->kind }}</div>
                            @endif
                        @else
                ...
                <li class="list-row">
                    <span class="text-[#fff9] font-semibold">Age:</span> <span>{{ $adopt->pet->age ?? 'N/A' }} years old</span>
                </li>
                <li class="list-row">
                ...
                <li class="list-row">
                    <span class="text-[#fff9] font-semibold">Active:</span>
                    <span>
                        @if(isset($adopt->pet->active) && $adopt->pet->active == 1)
                ...
                <li class="list-row">
                    ...
                    <span>
                        @if(isset($adopt->pet->adopted) && $adopt->pet->adopted == 1)
        ...
        {{-- Back Button --}}
        <div class="mt-6">
            <a href="{{ url('adoptions') }}" class="btn btn-outline btn-sm hover:bg-white/50 bg-white">
